...
exp_no = 1
logdir = 'runs6/TU_Experiment'+str(exp_no)
resize = (224,224)
SAMPLE_SIZE = 15000
...
# # loading data
# train_dataset = TUDataset(np.array(X_train[0:SAMPLE_SIZE]), np.array(y_train[0:SAMPLE_SIZE]), resize=(128,128))
train_dataloader = DataLoader(train_dataset,batch_size=1, shuffle=False, pin_memory=True, num_workers=2)
...
# model = torch.load('/home/anudeep/repos/Lane-Detection-Pytorch/scripts/saved_models/model-UnetResnet.pth')
...
for epoch in range(10):
    ...
    if bce_loss < best_loss:
        best_loss = bce_loss
        print('Best Model!')
    modelname = 'model-'+ str(exp_no) +'.pth'
